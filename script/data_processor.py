...
class DataProcessor:
    def __init__(self, in_config) -> None:
        ...
        self.fig_size:Tuple[int,int] = (15, 8)
        self.marker_size:int = 15
        self.line_width:float = 1.8
        self.text_size:int = 20
        self.fig_axs:Dict[int, Tuple[int,int]] = {2: (1,2),
                                                  3: (1,3),
                                                  4: (2,2),
                                                  5: (1,5),
                                                  6: (2,3),
    ...
    def get_val(self, x_index:str='num', y_index:str='succ'):
        if x_index == 'ins':
            return self.get_ins_val(y_index)
        elif x_index == 'num':
            return self.get_num_val(y_index)
        elif x_index == 'w':
            return self.get_w_val(y_index)
    ...
    def get_w_val(self, in_index:str):
        """Compute the success rate versus the numbers of agents
    ...
    def subplot_fig(self, x_index, y_index, in_axs, in_map_idx, in_map, in_result):
        ...
        for solver in self.config['solvers']:
            ...
            if self.config['plot_ci'] and len(_ci_) > 0:
                ...
                in_axs.fill_between(_x_, _lb_, _ub_, color=solver['color'], alpha=0.2)
